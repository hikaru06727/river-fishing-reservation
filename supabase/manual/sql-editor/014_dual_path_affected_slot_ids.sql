-- Phase 9c: get_affected_slot_ids_for_reservation dual-path（014 と同内容）
-- 012 以降・013 以降の環境で helper のみ更新する場合に使用。
-- expire_pending_reservations は 012 版のまま（helper 呼び出しのみ）で可。
