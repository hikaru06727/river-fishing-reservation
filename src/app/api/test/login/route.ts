...
/**
 * E2E テスト専用: メール受信を経由せず、既存ユーザーのセッションを発行して
 * Cookie にセットする。Playwright からのみ利用想定。
 *
 * body.type で "magiclink"（デフォルト）・"recovery" を指定できる。
 * "recovery" はパスワード再設定リンクをクリックした直後と同じ状態
 * （認証済み・/login/reset/confirm でパスワード変更可能な状態）を再現する
 * ため、Phase 20 のパスワード再設定フローの E2E テストで使う。
 *
...
export async function POST(request: Request) {
  ...
  if (linkError || !linkData.properties?.hashed_token) {
    return NextResponse.json(
      { error: linkError?.message ?? "Failed to generate link (does the user exist?)" },
      { status: 400 },
    );
  }
