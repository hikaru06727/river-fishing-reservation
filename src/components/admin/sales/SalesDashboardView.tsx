    ...
    <div className="space-y-6">
      <header className="space-y-2">
        ...
        <p className="text-xs text-muted">
          確定売上は決済完了（オンライン）または現金受領済みの金額です。売上見込みは確定予約のみを集計し、キャンセル・期限切れ・未確定（pending）予約は含みません。予約件数は確定・仮予約の合計です。
        </p>
      </header>
      ...
      <div className="grid gap-6 xl:grid-cols-3">
        <div className="xl:col-span-1">
          <SalesPaymentMethodBreakdown breakdown={report.paymentMethodBreakdown} />
        </div>
        <div className="space-y-6 xl:col-span-2">
          <SalesDailyTable rows={report.dailyBreakdown} />
        </div>
      </div>

      <div className="grid gap-6 lg:grid-cols-2">
        <SalesBusinessTable rows={report.businessBreakdown} />
        <SalesPlanTable rows={report.planBreakdown} />
      </div>
    </div>
  );
}
